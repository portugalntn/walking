...
export function DifficultyGauge({ level }: { level: GradeLevel }) {
  ...
  const shown = hover !== null ? hover : activeIndex;
  const g = grades[shown];

  // Needle always points to this product's grade.
  const needleAngle = 180 - (level - 0.5) * 36;
  const [nx, ny] = polar(cx, cy, needleAngle, rO - 16);
    ...
    >
      ...
      >
        ...
        <svg viewBox="0 0 300 178" width="100%" role="img" aria-label={`${t("difficultyHeading")}: ${g.name[locale]}`}>
            ...
            return (
              ...
              >
                <path
                  ...
                  fill={RAMP[i]}
                  fillOpacity={isActive ? 1 : isHover ? 0.92 : 0.5}
          ...
          {/* Needle to this product's grade */}
        ...
        {/* Legend (updates on hover, defaults to this product's grade) */}
        <div>
          <span
            style={{
